package wtf.opal.client.renderer.liquidglass.reglass;
...
public final class LiquidGlassPrecomputeRuntime {
    ...
    private static final Identifier VS_ID = Identifier.of("opal", "core/liquid_glass_fullscreen");
    private static final Identifier BLUR_ID = Identifier.of("opal", "program/liquid_glass_blur");
    ...
    private void ensurePipelines() {
        if (blurPipeline == null) {
            blurPipeline = RenderPipeline.builder()
                    .withLocation(Identifier.of("opal", "pipeline/liquid_glass_blur"))
        ...
        if (samplerInfoUbo == null) {
            samplerInfoUbo = RenderSystem.getDevice().createBuffer(() -> "opal liquid glass SamplerInfo (pre)", 130, 16);
        }

        int blurConfigSize = 16 + (MAX_RADIUS + 1) * 16;
        if (blurConfigUboX == null) {
            blurConfigUboX = RenderSystem.getDevice().createBuffer(() -> "opal liquid glass BlurConfig X", 130, blurConfigSize);
        }
        if (blurConfigUboY == null) {
            blurConfigUboY = RenderSystem.getDevice().createBuffer(() -> "opal liquid glass BlurConfig Y", 130, blurConfigSize);
        }
    }
    ...
    private void ensureTempTarget(int w, int h) {
        if (blurTempTex == null || blurTempTex.getWidth(0) != w || blurTempTex.getHeight(0) != h) {
            ...
            blurTempTex = RenderSystem.getDevice().createTexture("opal liquid glass blur temp", 12, TextureFormat.RGBA8, w, h, 1, 1);
    ...
    private void ensureOutputForRadius(int w, int h, int radius) {
        ...
        if (tex == null || tex.getWidth(0) != w || tex.getHeight(0) != h) {
            ...
            GpuTexture newTex = RenderSystem.getDevice().createTexture("opal liquid glass blurred r=" + radius, 12, TextureFormat.RGBA8, w, h, 1, 1);
    ...
    public void run() {
        ...
        for (int k = 0; k < max; k++) {
            ...
            try (RenderPass pass = ce.createRenderPass(() -> "opal liquid glass blur X r=" + radius, blurTempView, OptionalInt.empty())) {
            ...
            try (RenderPass pass = ce.createRenderPass(() -> "opal liquid glass blur Y r=" + radius, blurredViewByRadius.get(radius), OptionalInt.empty())) {
                pass.setPipeline(blurPipeline);
                RenderSystem.bindDefaultUniforms(pass);
                pass.setUniform("SamplerInfo", samplerInfoUbo);
